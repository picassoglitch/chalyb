...
First confirm the apply reached the end — fifteen secrets, each with a version:

```sh
gcloud secrets list --project=chalyb | tail -n +2 | wc -l   # 15
```
...
worker bearer token, the Drive encryption key, ChalyOBS's session-cookie
key and the HMAC secret that lets
ChalyClip's boto3 client reach the media bucket: nothing outside decides
...
**Placeholders you replace (seven).** Created holding the string `REPLACE_ME`
...
ChalyOBS does not use that DSN. Its web app reads the shared project through
supabase-js, so it needs the project's **secret key** — Supabase → Project
Settings → API Keys → Secret keys (`sb_secret_...`; the legacy `service_role`
JWT also works). Never the publishable/anon key:

```sh
printf '%s' "sb_secret_..." | gcloud secrets versions add supabase-secret-key --data-file=- --project=chalyb
```
